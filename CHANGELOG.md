...
## Unreleased
- **Added:** **`staleness_check.py`** — a project-plan freshness nudge (sibling to `drift_check.py`). Flags every active project that has an in-progress task but whose `resume-here` plan section is missing or trails the project's newest task activity by more than `STALENESS_DAYS` (default 7), then posts a single **idempotent** manager inbox alert (`from_agent='brain-staleness'`, prior unread marked read first). Read-only check pass; the scheduled path is a nudge (exits 0 unless `--strict`). Installed as the daily `brain-staleness-check.timer` (05:55). Closes the second half of the plan-drift gap that `drift_check.py` (invariant drift) left open.
- **Fixed:** `POST /session-overlay` (global scope) now **busts the `cfg()` cache** after committing, so a set/clear of the `SESSION_BRIEF_OVERLAY` house-rules overlay is reflected by `/bootstrap` immediately instead of lagging up to `_CFG_TTL` (30s) — and inconsistently across gunicorn workers. Previously only `POST /config` busted the cache, so the two config-write surfaces behaved differently; a fresh overlay read back correctly from `GET /session-overlay` (direct table read) while `/bootstrap` still injected the stale value, which looked like a failure but was cache lag.
